...
<xsl:template match="/">
       ...
       <xsl:variable name="MsgSubject">
			<xsl:value-of select="$ErrorIdentificationDoc/EmailIdentification/EmailDetails/Header/Subject/text()"/>
		</xsl:variable>
		
		<xsl:variable name="DEmailSubject" select="dp:variable('var://context/var/DynamicSubject')"/>
		
		<xsl:variable name="EmailSubject">
				  <xsl:value-of select="concat($DEmailSubject,'_',$MsgSubject)" /> 
			</xsl:variable>
        ...
        <xsl:variable name="EncSubject" select="dp:encode($EmailSubject,'URL')"/>
